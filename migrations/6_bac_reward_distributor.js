const { ebtcPools, INITIAL_EBTC_FOR_POOLS } = require('./pools');
...
module.exports = async (deployer, network, accounts) => {
  ...
  const initialCashAmount = unit.muln(INITIAL_EBTC_FOR_POOLS).toString();

  const cash = await Cash.deployed();
  const pools = ebtcPools.map(({ contractName }) =>
  ...
  console.log(
    `Deposited ${INITIAL_EBTC_FOR_POOLS} BAC to InitialCashDistributor.`
  );
